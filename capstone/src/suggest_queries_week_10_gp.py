...
def pickConfigForFunction(functionIndex: int) -> Dict[str, float | str]:
    # Week 10: final exploitation phase, 4 weeks remaining
    configMap: Dict[int, Dict[str, float | str]] = {
        1: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.98, "localStd": 0.050},
        2: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 1.00, "localStd": 0.006},
        3: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.98, "localStd": 0.012},
        4: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.98, "localStd": 0.015},
        5: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.00, "localStd": 0.000},  # special
        6: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-3, "localFrac": 0.80, "localStd": 0.040},
        7: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.95, "localStd": 0.020},
        8: {"acq": "ei",  "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.90, "localStd": 0.025},
    }
...
def suggestNextPoint(
    ...
    # topK=1 for F1 and F2 — single best centre only
    topK: int = 1 if functionIndex in (1, 2, 4) else min(5, xTrain.shape[0])
    topIndices: np.ndarray = np.argsort(yTrain)[-topK:]
    xCenters: np.ndarray = xTrain[topIndices]

    # F5 gets special boundary-pinned candidates
    if functionIndex == 5:
