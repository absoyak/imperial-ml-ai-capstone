...
def pickConfigForFunction(functionIndex: int) -> Dict[str, float | str]:
    configMap: Dict[int, Dict[str, float | str]] = {        
        1: {"acq": "ucb",     "kappa": 1.5,  "xi": 0.0,  "localFrac": 0.20, "localStd": 0.120},        
        2: {"acq": "ucb",     "kappa": 0.40, "xi": 0.0,  "localFrac": 0.85, "localStd": 0.010},       
        3: {"acq": "ei",      "kappa": 0.0,  "xi": 1e-4, "localFrac": 0.75, "localStd": 0.050},
        4: {"acq": "ucb",     "kappa": 0.50, "xi": 0.0,  "localFrac": 0.85, "localStd": 0.030},
        5: {"acq": "ei",      "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.92, "localStd": 0.010},
        6: {"acq": "ucb",     "kappa": 1.50, "xi": 0.0,  "localFrac": 0.25, "localStd": 0.100},
        7: {"acq": "ei",      "kappa": 0.0,  "xi": 1e-6, "localFrac": 0.85, "localStd": 0.020},
        8: {"acq": "ucb",     "kappa": 1.20, "xi": 0.0,  "localFrac": 0.80, "localStd": 0.025},
    }
...
def suggestNextPoint(
    ...
    # Top-K filtreleme: F2 ve F6 icin surrogate'i temizle
    k: int = TOP_K_FILTER.get(functionIndex, 0)
    if k > 0:
        xFit, yFit = filterTopK(xTrain, yTrain, k)
    else:
        xFit, yFit = xTrain, yTrain

    topK: int = 1 if functionIndex in (1, 2) else min(5, xFit.shape[0])
    ...
    lengthScale: float = 0.30 if functionIndex == 2 else baseLengthScale
    noiseLevel: float = 1e-7 if functionIndex == 2 else baseNoiseLevel
