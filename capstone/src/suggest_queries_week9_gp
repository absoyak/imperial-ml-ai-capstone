...
def pickConfigForFunction(functionIndex: int) -> Dict[str, float | str]:
    # Week 9: tight exploitation on proven regions, 5 weeks remaining
    configMap: Dict[int, Dict[str, float | str]] = {
        ...
        8: {"acq": "ei",      "kappa": 0.0,  "xi": 1e-6,  "localFrac": 0.85, "localStd": 0.030},
    }
...
def main() -> None:
    ...
    for functionIndex in range(1, 9):
        ...
        if info["acquisition"] == "spread":
            print(f"  spread score  : {info.get('spread', 0.0)}\n")
        else:
